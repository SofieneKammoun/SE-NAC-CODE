...
class Trainer:
    ...
    def decode_latents(self,layer, latents):
        ...
        probs=torch.nn.functional.softmax(-dist,dim=1)
        GumbelSofmax =torch.distributions.relaxed_categorical.RelaxedOneHotCategorical(
            torch.tensor([self.tau], device=self.gpu_id) , probs=probs)
        samples = GumbelSofmax.rsample()
        indices_ = rearrange(torch.argmax(samples, dim=1), "(b t) -> b t", b=latents.size(0))
        z_q = layer.decode_code(indices_)
        probs= rearrange(GumbelSofmax.probs, "(b t) f -> b t f" , b=latents.size(0))
        return z_q, indices_, probs
    ...
    def Tokenize(self,model,x):
        ...
        for i ,layer in enumerate(model.module.quantizer.quantizers[:self.Nq]):
            ...
            z_q, indices, probs = self.decode_latents(layer,z_in)# z_q : (B x D x T) ## indices: (B x T)
        ...
        return z_out , z ,logits

    def process_batch_train_audio(self, batch):
        with torch.no_grad():
            x=self.Tokenize( self.model_c,batch[0].to(self.gpu_id).detach())[1]
        logits=self.Tokenize( self.model,batch[1].to(self.gpu_id))[-1]
        x = rearrange(x, "b t n-> b (t n)")
        lables= x.long()
        logits = rearrange( torch.stack(logits, dim=2), 'b t n f  -> b f (t n)')
        loss =torch.nn.functional.cross_entropy(logits, lables)
        return loss
    ...
    def _denoise_validation(self,epoch):
        ...
        with torch.no_grad():
            ...
            for i , data in enumerate(tqdm.tqdm(self.val_dataset, desc="Validation : Denoising")):
                ...
                y_, _ ,_= self.Tokenize(self.model, data[1].to(self.gpu_id))

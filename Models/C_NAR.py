...
class C_NAR_Model(nn.Module):
    ...
    def forward(self, embeds, clean_embeds=None , return_loss = False,gen=False):
        ...
        labels = clean_embeds
        # var=exp(log_var)
        # stack_var =reduce(log_var, 'b t f -> b t', 'sum') 
        # diff= sqr(lables - preds) / var
        # stack_diff=reduce(diff, 'b t f -> b t', 'sum') 
        # loss = -0.5 * stack_diff - 0.5 * stack_var

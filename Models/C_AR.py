import torch
from torch import nn 
from tqdm import tqdm
from einops import rearrange , repeat
from Models.Conformer import Conformer


def exists(val):
    return val is not None

def default(val, d):
    return val if exists(val) else d

def calc_same_padding(kernel_size):
    pad = kernel_size // 2
    return (pad, pad - (kernel_size + 1) % 2)


def remainder_to_mult(num, mult):
    return (mult - num % mult) % mult

def log(t, eps = 1e-20):
    return torch.log(t.clamp(min = eps))

def gumbel_noise(t):
    noise = torch.zeros_like(t).uniform_(0, 1)
    return -log(-log(noise))

def gumbel_sample(t, temperature = 1., dim = -1):
    return ((t / temperature) + gumbel_noise(t)).argmax(dim = dim)

def top_k(logits, thres = 0.5):
    num_logits = logits.shape[-1]
    k = max(int((1 - thres) * num_logits), 1)
    val, ind = torch.topk(logits, k)
    probs = torch.full_like(logits, float('-inf'))
    probs.scatter_(1, ind, val)
    return probs


def FeedForward(*, dim, mult = 4, dropout = 0.):
    return nn.Sequential(
        nn.LayerNorm(dim),
        nn.Linear(dim, dim * mult),
        nn.GELU(),
        nn.Dropout(dropout),
        nn.Linear(dim * mult, dim)
    )

class C_AR_Model(nn.Module):
    def __init__(
        self,
        *,
        # num_tokens,
        input_dim,
        dim,
        max_seq_len,
        N_layers,
        dim_head = 64,
        heads = 8,
        attn_dropout = 0.,
        ff_mult = 4,
        ff_dropout = 0.,
        pad_id = 0,
        conv_kernel_size=10
    ):
        super().__init__()
        
        self.max_seq_len = max_seq_len
        
        self.dim = dim
        self.input_dim=input_dim
        # PE_t
        self.spatial_pos_emb = nn.Embedding(int(2*max_seq_len )+ 1,self.dim) # account for a boundary case
        self.noise_transformer = Conformer(
            dim = self.dim,
            layers = N_layers,
            dim_head = dim_head,
            heads = heads,
            conv_kernel_size = conv_kernel_size,
            attn_dropout = attn_dropout,
            ff_dropout = ff_dropout,
            ff_mult = ff_mult,
            conv_causal=True
        )
        self.spatial_start_token = nn.Parameter(torch.randn(self.dim))


        self.input_layer  = nn.Linear(self.input_dim, self.dim)
        # self.output_layer = nn.Linear(self.dim, self.input_dim)
        self.output_layer = nn.Sequential(
                FeedForward(dim=self.dim),
                nn.Linear(self.dim, self.input_dim)
                )
    def generate(self, noisy ):
        max_len = self.max_seq_len
        B, T, D = noisy.shape
        device = next(self.parameters()).device
    
        full_seq = torch.empty((B, 0, D), dtype=torch.float, device=device)
    
        # Process the noisy input in chunks
        for start in range(0, T, max_len):
            end = min(start + max_len, T)
            noisy_chunk = noisy[:, start:end, :]  # shape (B, chunk_len, D)
            chunk_len = noisy_chunk.shape[1]
    
            generated_chunk = torch.empty((B, 0, D), dtype=torch.float, device=device)
    
            for i in tqdm(range(chunk_len), desc=f"Generating chunk {start//max_len + 1}"):
                predicted_embed = self.forward(
                    embeds=noisy_chunk, clean_embeds=generated_chunk
                )[:, -1]  # shape (B, D)
                predicted_embed = rearrange(predicted_embed, 'b d -> b 1 d')
                generated_chunk = torch.cat((generated_chunk, predicted_embed), dim=1)
    
            # Append this chunk’s output to the full sequence
            full_seq = torch.cat((full_seq, generated_chunk), dim=1)

        return full_seq

    def forward(self, embeds, quant_clean,clean_embeds=None,return_loss = False):
        assert embeds.ndim == 3
        b, T, _, device = *embeds.shape, embeds.device
        _, t, _ = quant_clean.shape
        assert T <= (self.max_seq_len + 1), 'spatial dimension is greater than the max_seq_len set'

        # Stack the embedded tokens along the depth dimension
        tokens = self.input_layer(embeds)
        clean_tokens = self.input_layer(quant_clean)

        spatial_pos = self.spatial_pos_emb(torch.arange(t+T+1, device = device))
        

         # spatial tokens is tokens with depth pos reduced along depth dimension + spatial positions
        
        spatial_tokens = torch.cat((
            tokens,
            repeat(self.spatial_start_token, 'f -> b 1 f', b = b),
            clean_tokens
        ), dim = -2)
        spatial_tokens = spatial_tokens + spatial_pos 
        spatial_tokens = self.noise_transformer(spatial_tokens)[:,T:,:]
        out_embeds = self.output_layer(spatial_tokens)
        if not return_loss :
           
            return out_embeds#[:, 1:]
        
        preds = out_embeds[:, :-1]
        
        labels = clean_embeds
        
        loss = torch.nn.functional.mse_loss(preds, labels)
        return loss

